...
- In customer journey stories, define `Discovery` as how the customer discovers the product or offer before active use. Use awareness and acquisition channels such as search, referrals, sales outreach, partner ecosystems, app stores, procurement shortlists, internal enablement, or brand touchpoints. Define `Evaluation` as how the customer decides whether the product is right for them after discovery and before trial, using alternatives, fit, value, trust, cost, adoption effort, and risk. Do not use `Discovery` or `Evaluation` for search, browsing, or task execution inside the product.
