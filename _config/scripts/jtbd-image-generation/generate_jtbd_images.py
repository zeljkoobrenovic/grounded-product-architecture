#!/usr/bin/env python3
"""Generate JTBD and JTBD-step images for product-domain customer models."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate JTBD images for all product-domain customer datasets."
    )
    ...
    parser.add_argument("--background", default="opaque", help="Image background mode.")
    parser.add_argument(
        "--skip-existing",
        action="store_true",
...
def build_targets_for_file(path: Path, inspiration: str) -> tuple[list[GenerationTarget], Any]:
    ...
    for group in payload:
        ...
        for customer in customers:
            ...
            for job in jobs:
                ...
                job_filename = make_job_filename(customer_id, job_id, DEFAULT_OUTPUT_FORMAT)
                ...
                for step_index, step in enumerate(steps, start=1):
                    if not isinstance(step, dict):
                        continue
                    step_filename = make_step_filename(
                        customer_id, job_id, step_index, DEFAULT_OUTPUT_FORMAT
                    )
...
def call_openai_image_api(
    ...
    if not image_b64:
        ...
        raise RuntimeError(f"OpenAI response did not contain image data: {payload}")
...
def main() -> int:
    ...
    if not args.json_only and not args.dry_run and not api_key:
        print("OPENAI_API_KEY is required unless --json-only or --dry-run is used.", file=sys.stderr)
        return 2
    ...
    for customer_file in customer_files:
        ...
        targets, payload = build_targets_for_file(customer_file, inspiration)
        json_changed = payload_text(payload) != original_text

        for target in targets:
            extension = args.output_format
            expected_name = (
                make_job_filename(target.customer_id, str(target.job.get("id") or ""), extension)
                if target.level == "job"
            ...
            expected_path = target.image_path.with_suffix(f".{extension}")
            ...
            if target.media_src != expected_src:
                ...
                alt = target.alt
                if target.level == "job":
                    if upsert_image_media(target.job, expected_src, title, alt):
                        json_changed = True
                elif target.step is not None and upsert_image_media(target.step, expected_src, title, alt):
                    json_changed = True
            ...
            if should_generate:
                ...
                if not args.dry_run:
                    ...
                    write_image(target.image_path, image_bytes, dry_run=False)
                else:
