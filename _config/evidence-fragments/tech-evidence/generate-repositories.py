...
DEFAULT_INPUT = SCRIPT_DIR.parent.parent / "raw-data" / "sokrates" / "repositories.json"
DEFAULT_OUTPUT = SCRIPT_DIR / "repositories.json"
...
def build_summary(repository: dict) -> str:
    ...
    test_ratio = round((test_loc / main_loc) * 100) if main_loc else 0

    return (
        f"{main_loc:,} main LOC, {test_loc:,} test LOC ({test_ratio}% of main), "
        f"{generated_loc:,} generated LOC, {build_loc:,} build/deploy LOC; "
        f"{commits_90d} commits and {contributors_90d} active contributors in the last 90 days; "
        f"latest commit on {latest_commit}."
    )


def build_links(repository: dict) -> list[dict]:
    ...
    if sokrates.get("htmlReportsRoot"):
        links.append({"label": "Sokrates HTML Reports", "url": sokrates["htmlReportsRoot"]})
    if sokrates.get("analysisResultsPath"):
        links.append({"label": "Sokrates Analysis Results", "url": sokrates["analysisResultsPath"]})
    return links


def build_fragment(repository: dict) -> dict:
    ...
    tags = [
        "source-code",
        "repository-analysis",
        f"owner:{owner.lower()}" if owner else "owner:unknown",
        size_tag(main_loc),
        activity_tag(commits_90d),
        freshness_tag(repository.get("latestCommitDate") or ""),
    ]
...
def build_output(repositories: list[dict], input_path: Path) -> dict:
    fragments = [build_fragment(repository) for repository in repositories]
    return {
        "config": {
...
def main() -> None:
    ...
    output = build_output(repositories, args.input)
